...
jaydb::pipe::pipe(bool close_on_exec) {
...
int jaydb::pipe::release_read() { return std::exchange(fds_[read_fd], -1); }

int jaydb::pipe::release_write() { return std::exchange(fds_[write_fd], -1); }

void jaydb::pipe::close_read() {
    if (fds_[read_fd] != -1) {
        close(fds_[read_fd]);
        fds_[read_fd] = -1;
    }
...
std::vector<std::byte> jaydb::pipe::read() {
    char buf[1024];
    int chars_read;
...
void jaydb::pipe::write(std::byte *from, std::size_t bytes) {
